...
const Hero = React.memo(function Hero({ restaurant }: HeroProps) {
  ...
  // Stable identity: the visual design must never interfere with the
  // restaurant's existing Hero slider. Use the configured slide id when
  // available, falling back to its position.
  const currentSlideKey = String(
    currentSlideData?.id ?? currentSlide
  );
  ...
  return (
    ...
    >
      ...
      <AnimatePresence mode="wait">
        <motion.div
          key={`hero-media-${currentSlideKey}`}
          initial={{
            opacity: 0,
            scale: 1.08,
          }}
    ...
    ease: "easeOut",
      ...
      >
        <div className="wolf-hero-copy max-w-4xl pt-20 sm:pt-24 lg:pt-12">
          ...
          {/* -------------------------------------------------------------- */}
          {/* SLIDE CONTROLS — existing slider, design-independent             */}
          {/* -------------------------------------------------------------- */}

          {slides.length > 1 && (
            <div className="wolf-hero-slider-controls mt-5 flex items-center gap-2">
              <button
                type="button"
                onClick={previousSlide}
                aria-label="Slide anterior"
                className="h-9 w-9 rounded-full border border-white/15 bg-black/20 text-white/80 backdrop-blur-md transition hover:bg-white/10"
              >
                ←
              </button>
              <button
